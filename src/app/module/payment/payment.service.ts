...
const handleStripeHookEvent=async(event:Stripe.Event)=>{
    ...
    if(existingPayment){
        ...
        return {message:`Event with id ${event.id} already processed`};
    }
    ...
    switch(event.type){
        case "checkout.session.completed":{
            ...
            console.log(`Payment for appointment ${appointmentId} is ${session.payment_status}`);
            break;
        }
    ...
    return {message:`Webhook Event with id ${event.id} processed`};
}
